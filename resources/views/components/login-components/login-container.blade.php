<div class="relative z-10 flex justify-center items-center w-full h-full bg-[rgba(237,132,56,0.6)] backdrop-blur-sm">
    <div class="bg-[#ea7025] text-white rounded-xl p-8 w-full max-w-md shadow-lg border border-white/10 mx-4">
        <h1 class="text-3xl font-bold text-center mb-2">Bienvenue !</h1>
        <p class="text-center text-gray-200 text-sm mb-6">Connectez-vous à votre compte.</p>
        ...
        <form method="POST" action="{{route('login')}}" class="space-y-5">
            ...
            <div>
                ...
                <div class="relative">
                    <input id="email" name="email" type="email" value="{{ old('email') }}" required autofocus
                        class="w-full px-4 py-2 bg-white text-gray-800 rounded-md placeholder-gray-400 outline-none @error('email') border border-red-500 @enderror"
            ...
            <div class="flex items-center justify-between text-sm text-gray-200">
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="remember" id="remember" class="accent-orange-500" {{ old('remember') ? 'checked' : '' }}>
                    <span>Se souvenir de moi</span>
                </label>
                <a href="" class="text-[#91341b] font-semibold hover:underline">Mot de passe oublié?</a>
            </div>
            ...
            <p class="text-center text-sm mt-4 text-gray-300">
                ...
                <a href="{{ route('register') }}" class="text-[#91341b] font-semibold hover:underline">Inscrivez-vous ici</a>
            </p>
        </form>
    </div>
</div>
