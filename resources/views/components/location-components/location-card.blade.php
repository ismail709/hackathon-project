<!-- LOCATION CARDS + PAGINATION WRAPPER -->
<div class="flex-1 flex flex-col items-center">

    <!-- LOCATION CARDS GRID FETCH 6 PER PAGE -->
    <div class="w-full grid grid-cols-1 md:grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-10">
        <!-- CARD 1 -->
        <div data-aos="slide-up"
            class="bg-white rounded-xl overflow-hidden shadow-md flex flex-col items-start text-left border border-[#f5d0c4]">
            
            <!-- Image -->
            <img src="{{ asset('assets/location-image.png') }}" alt="location"
                class="w-full h-full object-cover rounded-t-xl ">

            <!-- Content -->
            <div class="p-4 w-full space-y-2">
                <!-- Title -->
                <p class="text-lg text-[#1d1d1d] font-semibold">Lorem Ipsum is simply dummy text.</p>

                <!-- Location & Capacity -->
                <div class="flex items-center justify-between text-sm text-[#91341b] font-medium">
                    <span><i class="fa-solid fa-location-dot mr-1"></i> Casablanca</span>
                    <span><i class="fa-solid fa-users mr-1"></i> 2 personnes</span>
                </div>

                <!-- Price -->
                <p class="text-lg font-bold text-[#1d1d1d]">200$</p>
            </div>

            <!-- Button -->
            <div class="px-4 pb-4 w-full">
                <button
                    class="w-full bg-[#db571b] text-white text-lg text-base px-6 py-3 rounded-md font-medium hover:opacity-90 transition cursor-pointer">
                    Voir
                </button>
            </div>
        </div>

        <!-- CARD 2 -->
        <div data-aos="slide-up"
            class="bg-white rounded-xl overflow-hidden shadow-md flex flex-col items-start text-left border border-[#f5d0c4]">
            
            <!-- Image -->
            <img src="{{ asset('assets/location-image.png') }}" alt="location"
                class="w-full h-full object-cover rounded-t-xl ">

            <!-- Content -->
            <div class="p-4 w-full space-y-2">
                <!-- Title -->
                <p class="text-lg text-[#1d1d1d] font-semibold">Lorem Ipsum is simply dummy text.</p>

                <!-- Location & Capacity -->
                <div class="flex items-center justify-between text-sm text-[#91341b] font-medium">
                    <span><i class="fa-solid fa-location-dot mr-1"></i> Casablanca</span>
                    <span><i class="fa-solid fa-users mr-1"></i> 2 personnes</span>
                </div>

                <!-- Price -->
                <p class="text-lg font-bold text-[#1d1d1d]">200$</p>
            </div>

            <!-- Button -->
            <div class="px-4 pb-4 w-full">
                <button
                    class="w-full bg-[#db571b] text-white text-lg text-base px-6 py-3 rounded-md font-medium hover:opacity-90 transition cursor-pointer">
                    Voir
                </button>
            </div>
        </div>

        <!-- CARD 3 -->
        <div data-aos="slide-up"
            class="bg-white rounded-xl overflow-hidden shadow-md flex flex-col items-start text-left border border-[#f5d0c4]">
            
            <!-- Image -->
            <img src="{{ asset('assets/location-image.png') }}" alt="location"
                class="w-full h-full object-cover rounded-t-xl ">

            <!-- Content -->
            <div class="p-4 w-full space-y-2">
                <!-- Title -->
                <p class="text-lg text-[#1d1d1d] font-semibold">Lorem Ipsum is simply dummy text.</p>

                <!-- Location & Capacity -->
                <div class="flex items-center justify-between text-sm text-[#91341b] font-medium">
                    <span><i class="fa-solid fa-location-dot mr-1"></i> Casablanca</span>
                    <span><i class="fa-solid fa-users mr-1"></i> 2 personnes</span>
                </div>

                <!-- Price -->
                <p class="text-lg font-bold text-[#1d1d1d]">200$</p>
            </div>

            <!-- Button -->
            <div class="px-4 pb-4 w-full">
                <button
                    class="w-full bg-[#db571b] text-white text-lg text-base px-6 py-3 rounded-md font-medium hover:opacity-90 transition cursor-pointer">
                    Voir
                </button>
            </div>
        </div>

        <!-- CARD 4 -->
        <div data-aos="slide-up"
            class="bg-white rounded-xl overflow-hidden shadow-md flex flex-col items-start text-left border border-[#f5d0c4]">
            
            <!-- Image -->
            <img src="{{ asset('assets/location-image.png') }}" alt="location"
                class="w-full h-full object-cover rounded-t-xl ">

            <!-- Content -->
            <div class="p-4 w-full space-y-2">
                <!-- Title -->
                <p class="text-lg text-[#1d1d1d] font-semibold">Lorem Ipsum is simply dummy text.</p>

                <!-- Location & Capacity -->
                <div class="flex items-center justify-between text-sm text-[#91341b] font-medium">
                    <span><i class="fa-solid fa-location-dot mr-1"></i> Casablanca</span>
                    <span><i class="fa-solid fa-users mr-1"></i> 2 personnes</span>
                </div>

                <!-- Price -->
                <p class="text-lg font-bold text-[#1d1d1d]">200$</p>
            </div>

            <!-- Button -->
            <div class="px-4 pb-4 w-full">
                <button
                    class="w-full bg-[#db571b] text-white text-lg text-base px-6 py-3 rounded-md font-medium hover:opacity-90 transition cursor-pointer">
                    Voir
                </button>
            </div>
        </div>

        <!-- CARD 5 -->
        <div data-aos="slide-up"
            class="bg-white rounded-xl overflow-hidden shadow-md flex flex-col items-start text-left border border-[#f5d0c4]">
            
            <!-- Image -->
            <img src="{{ asset('assets/location-image.png') }}" alt="location"
                class="w-full h-full object-cover rounded-t-xl ">

            <!-- Content -->
            <div class="p-4 w-full space-y-2">
                <!-- Title -->
                <p class="text-lg text-[#1d1d1d] font-semibold">Lorem Ipsum is simply dummy text.</p>

                <!-- Location & Capacity -->
                <div class="flex items-center justify-between text-sm text-[#91341b] font-medium">
                    <span><i class="fa-solid fa-location-dot mr-1"></i> Casablanca</span>
                    <span><i class="fa-solid fa-users mr-1"></i> 2 personnes</span>
                </div>

                <!-- Price -->
                <p class="text-lg font-bold text-[#1d1d1d]">200$</p>
            </div>

            <!-- Button -->
            <div class="px-4 pb-4 w-full">
                <button
                    class="w-full bg-[#db571b] text-white text-lg text-base px-6 py-3 rounded-md font-medium hover:opacity-90 transition cursor-pointer">
                    Voir
                </button>
            </div>
        </div>

        <!-- CARD 6 -->
        <div data-aos="slide-up"
            class="bg-white rounded-xl overflow-hidden shadow-md flex flex-col items-start text-left border border-[#f5d0c4]">
            
            <!-- Image -->
            <img src="{{ asset('assets/location-image.png') }}" alt="location"
                class="w-full h-full object-cover rounded-t-xl ">

            <!-- Content -->
            <div class="p-4 w-full space-y-2">
                <!-- Title -->
                <p class="text-lg text-[#1d1d1d] font-semibold">Lorem Ipsum is simply dummy text.</p>

                <!-- Location & Capacity -->
                <div class="flex items-center justify-between text-sm text-[#91341b] font-medium">
                    <span><i class="fa-solid fa-location-dot mr-1"></i> Casablanca</span>
                    <span><i class="fa-solid fa-users mr-1"></i> 2 personnes</span>
                </div>

                <!-- Price -->
                <p class="text-lg font-bold text-[#1d1d1d]">200$</p>
            </div>

            <!-- Button -->
            <div class="px-4 pb-4 w-full">
                <button
                    class="w-full bg-[#db571b] text-white text-lg text-base px-6 py-3 rounded-md font-medium hover:opacity-90 transition cursor-pointer">
                    Voir
                </button>
            </div>
        </div>

         <!-- CARD 3 -->
        <div data-aos="slide-up"
            class="bg-white rounded-xl overflow-hidden shadow-md flex flex-col items-start text-left border border-[#f5d0c4]">
            
            <!-- Image -->
            <img src="{{ asset('assets/location-image.png') }}" alt="location"
                class="w-full h-full object-cover rounded-t-xl ">

            <!-- Content -->
            <div class="p-4 w-full space-y-2">
                <!-- Title -->
                <p class="text-lg text-[#1d1d1d] font-semibold">Lorem Ipsum is simply dummy text.</p>

                <!-- Location & Capacity -->
                <div class="flex items-center justify-between text-sm text-[#91341b] font-medium">
                    <span><i class="fa-solid fa-location-dot mr-1"></i> Casablanca</span>
                    <span><i class="fa-solid fa-users mr-1"></i> 2 personnes</span>
                </div>

                <!-- Price -->
                <p class="text-lg font-bold text-[#1d1d1d]">200$</p>
            </div>

            <!-- Button -->
            <div class="px-4 pb-4 w-full">
                <button
                    class="w-full bg-[#db571b] text-white text-lg text-base px-6 py-3 rounded-md font-medium hover:opacity-90 transition cursor-pointer">
                    Voir
                </button>
            </div>
        </div>

        <!-- CARD 4 -->
        <div data-aos="slide-up"
            class="bg-white rounded-xl overflow-hidden shadow-md flex flex-col items-start text-left border border-[#f5d0c4]">
            
            <!-- Image -->
            <img src="{{ asset('assets/location-image.png') }}" alt="location"
                class="w-full h-full object-cover rounded-t-xl ">

            <!-- Content -->
            <div class="p-4 w-full space-y-2">
                <!-- Title -->
                <p class="text-lg text-[#1d1d1d] font-semibold">Lorem Ipsum is simply dummy text.</p>

                <!-- Location & Capacity -->
                <div class="flex items-center justify-between text-sm text-[#91341b] font-medium">
                    <span><i class="fa-solid fa-location-dot mr-1"></i> Casablanca</span>
                    <span><i class="fa-solid fa-users mr-1"></i> 2 personnes</span>
                </div>

                <!-- Price -->
                <p class="text-lg font-bold text-[#1d1d1d]">200$</p>
            </div>

            <!-- Button -->
            <div class="px-4 pb-4 w-full">
                <button
                    class="w-full bg-[#db571b] text-white text-lg text-base px-6 py-3 rounded-md font-medium hover:opacity-90 transition cursor-pointer">
                    Voir
                </button>
            </div>
        </div>

        <!-- CARD 5 -->
        <div data-aos="slide-up"
            class="bg-white rounded-xl overflow-hidden shadow-md flex flex-col items-start text-left border border-[#f5d0c4]">
            
            <!-- Image -->
            <img src="{{ asset('assets/location-image.png') }}" alt="location"
                class="w-full h-full object-cover rounded-t-xl ">

            <!-- Content -->
            <div class="p-4 w-full space-y-2">
                <!-- Title -->
                <p class="text-lg text-[#1d1d1d] font-semibold">Lorem Ipsum is simply dummy text.</p>

                <!-- Location & Capacity -->
                <div class="flex items-center justify-between text-sm text-[#91341b] font-medium">
                    <span><i class="fa-solid fa-location-dot mr-1"></i> Casablanca</span>
                    <span><i class="fa-solid fa-users mr-1"></i> 2 personnes</span>
                </div>

                <!-- Price -->
                <p class="text-lg font-bold text-[#1d1d1d]">200$</p>
            </div>

            <!-- Button -->
            <div class="px-4 pb-4 w-full">
                <button
                    class="w-full bg-[#db571b] text-white text-lg text-base px-6 py-3 rounded-md font-medium hover:opacity-90 transition cursor-pointer">
                    Voir
                </button>
            </div>
        </div>
    </div>

    <!-- location PAGINATION -->
    <x-pagination/>
</div>